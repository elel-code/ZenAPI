...
ZenAPI 采用单窗口多页面架构。当前 Slint 迁移阶段暂不渲染全局顶栏
和全局底部状态栏；后续顶栏需要按参考 UI 单独重构，未接线的全局
操作、品牌标题、小圆点状态和底部说明文字都不应提前放回界面。
...
|------|------|
| 当前状态 | 不在 Slint 主壳中渲染 |
| 后续要求 | 单独按参考 UI 重构后再加入 |
| 禁止项 | 未接线全局操作、Mock 开关、品牌占位、小圆点状态、装饰性文字 |
...
|------|------|
| 当前状态 | 不在 Slint 主壳中渲染 |
| 状态呈现 | 放在对应页面或面板内，例如 Mock Manager、Response、Collection 状态行 |
| 禁止项 | 全局堆叠 Mock/route/busy/Ready 等说明文字 |
...
- 侧边栏导航点击 → 同步更新导航选中状态和实际页面索引，避免人为延迟造成首次切换卡顿
- 页面级 ScrollView 常驻在主内容容器内，只切换 `visible`，避免导航时销毁/重建复杂页面导致首次切换卡顿
- 静态只读代码示例使用轻量文本代码块，只有可编辑内容、动态响应体和生成结果使用 CodeEditor
- 不实现页面间滑动动画
- 移动端底部导航切换同逻辑
