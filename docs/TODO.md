...
> UI 规范参考 DESIGN.md，视觉对标以阶段 0 调研为补充。

- [x] 1.1 搭建 GPUI 应用入口 (`main.rs`)，通过 Zed 官方 `gpui_platform::application()` 启动；不再保留 bundled font 资产
- [x] 1.2 实现全局 UI 主题系统: 颜色 token、HTTP 方法色映射、状态色调映射（遵循 DESIGN.md 规范，参考 Insomnia 深色主题的色调层次）
...
- [ ] 1.6 实现底部状态栏: Mock 服务器开关、端口显示、全局状态指示
- [~] 1.7 实现标签页系统: 已有 40px 面板标题行，激活下划线和内容区切换仍需补齐
...
- [ ] 2.1 将 `src/openapi/` 模块适配 GPUI 数据流（状态所有权、事件模型）
- [ ] 2.2 实现 Import 弹窗 UI: 文件路径输入、Enter 确认、导入进度状态
- [ ] 2.3 实现路由列表渲染: 方法标记（固定宽度文本，非填充徽章）、路径文本、可选摘要行
- [ ] 2.4 实现路由选中态: 3px 主色左边标记 + 行背景高亮
- [ ] 2.5 实现路由过滤: 按方法/路径/摘要过滤，过滤不影响全局操作
- [ ] 2.6 实现导入新规格时自动停止运行中的 Mock 服务
- [ ] 2.7 验证: 导入测试用 OpenAPI JSON/YAML 文件，路由列表正确显示
...
- [ ] 3.1 实现请求方法选择器: GET/POST/PUT/PATCH/DELETE/OPTIONS/HEAD，下拉或分段控件
- [ ] 3.2 实现 URL 地址栏: 单行输入、Enter 发送、与路由选择联动
- [ ] 3.3 实现请求 Headers 编辑面板: 键值对表格、批量编辑、常用 Header 提示
- [ ] 3.4 实现 Query Params 编辑面板: 键值对表格、自动拼接到 URL
- [ ] 3.5 实现 Request Body 编辑面板:
  - [ ] 3.5.1 支持 `none` / `form-data` / `x-www-form-urlencoded` / `raw` / `binary` 类型切换
  - [ ] 3.5.2 `raw` 模式下支持 JSON/XML/Text/HTML 语法高亮（预留 `syntect` 集成点，参考 Insomnia 的编辑器体验）
  - [ ] 3.5.3 `form-data` 支持文本字段和文件选择
- [ ] 3.6 实现 Authorization 面板:
  - [ ] 3.6.1 Bearer Token
  - [ ] 3.6.2 Basic Auth
  - [ ] 3.6.3 API Key (Header/Query)
  - [ ] 3.6.4 OAuth 2.0（远期，参考 Yaak 的插件化认证方案）
  - [ ] 3.6.5 JWT（远期）
- [ ] 3.7 实现发送请求: 通过 `reqwest` 发送，显示加载状态，按钮禁用防重入
- [ ] 3.8 实现响应查看器:
  - [ ] 3.8.1 状态码 + 耗时 + 响应大小元数据显示（260px 右对齐，14px 右内边距）
  - [ ] 3.8.2 响应体 Pretty 模式: JSON 格式化、缩进、折叠（参考 Insomnia 的响应折叠效果）
  - [ ] 3.8.3 响应体 Raw 模式: 原始文本
  - [ ] 3.8.4 Response Headers 子标签页
...
- [ ] 4.1 将 `src/mock_server/` 模块适配 GPUI 数据流
- [ ] 4.2 实现 Mock 开关控件: 启动/停止按钮、端口显示、启用/禁用状态
- [ ] 4.3 实现 CORS 默认开启，无需额外配置
- [ ] 4.4 增强 Mock 响应生成:
  - [ ] 4.4.1 基于 OpenAPI Schema 的 JSON 响应生成（已有）
  - [ ] 4.4.2 Schema-aware 假数据生成: 字段名启发式 (`email` → 生成 email, `name` → 生成姓名)
  - [ ] 4.4.3 支持响应示例 (`examples`) 优先于 Schema 生成
- [ ] 4.5 实现 Mock 请求日志: 记录到达的请求，在 UI 中展示最近 N 条
- [ ] 4.6 验证: 启动 Mock 服务，浏览器无 CORS 错误访问
...
- [ ] 5.1 设计变量数据模型: 变量名、当前值、初始值、作用域（全局/环境），参考 Bruno 的文件系统变量
- [ ] 5.2 实现变量解析引擎: `{{variableName}}` 语法替换
- [ ] 5.3 实现环境管理 UI:
  - [ ] 5.3.1 环境列表与创建/删除
  - [ ] 5.3.2 环境变量编辑器（键值对表格）
  - [ ] 5.3.3 当前活跃环境指示
- [ ] 5.4 实现全局变量管理 UI
- [ ] 5.5 变量集成到请求构建器: URL/Headers/Body 中均支持 `{{var}}` 替换
- [ ] 5.6 验证: 创建环境变量，在请求 URL 中使用，确认替换正确
...
- [ ] 6.1 设计集合数据模型: 集合 → 文件夹 → 请求的三级结构
- [ ] 6.2 实现集合序列化/反序列化 (JSON 格式，兼容 Postman Collection v2.1 格式)
...
- [ ] 7.1 设计历史记录数据模型: 时间戳、请求详情、响应摘要
- [ ] 7.2 实现请求自动记录: 每次发送请求后自动保存
- [ ] 7.3 实现历史侧边栏视图: 按时间倒序、搜索过滤
- [ ] 7.4 实现历史条目操作: 点击恢复请求、删除单条/批量清除
- [ ] 7.5 验证: 发送请求后在历史列表中可见
...
- [ ] 8.1 设计代码生成器架构: 语言模板 + 请求数据填充
- [ ] 8.2 实现首批语言支持: cURL / Python (requests) / JavaScript (fetch) / Rust (reqwest) / Go (net/http)
- [ ] 8.3 实现代码片段 UI: 语言选择下拉、代码预览、一键复制
- [ ] 8.4 验证: 构建请求，生成 cURL 命令，在终端执行验证
...
> **目标**: UI 细节对齐 DESIGN.md 和调研结果，稳定性和性能优化，准备首次发布。

- [ ] 13.1 UI 审查与 DESIGN.md 对齐: 逐项检查颜色、间距、字体、交互是否符合设计规范
- [ ] 13.2 UI 审查与竞品调研对齐: 确保核心交互流畅度不低于 Hoppscotch，功能深度对标 Postman 核心
- [ ] 13.3 性能优化: 大规格文件导入流畅度、响应体渲染性能、启动时间 <1s
- [ ] 13.4 错误处理完善: 网络错误、文件解析错误的用户友好提示
- [ ] 13.5 可执行文件大小优化: 目标 ~10 MB
- [ ] 13.6 跨平台构建验证: Windows / macOS / Linux
- [ ] 13.7 编写用户文档和使用指南
- [ ] 13.8 发布 v0.1.0
...
| HTTP 客户端 | 已实现基础 | reqwest |
| Mock 服务器 | 已实现基础 | Axum, permissive CORS |
