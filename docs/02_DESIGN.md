...
- **顶部栏**: 当前 Slint 迁移阶段暂不渲染；后续需要按参考 UI 单独重构后再加入。
- **左侧边栏**: 240px 导航，承载页面切换；移动端由底部导航替代。
- **主工作区**: 页面内容独立滚动；Request 页保持集合侧栏 + 请求编辑器 + 响应面板三列。
- **底部状态栏**: 当前不渲染全局状态栏；状态信息应出现在对应页面或面板内，避免堆叠说明文字。
...
- Send 按钮点击发送。
- 发送期间，地址栏整体禁用。
- 加载状态通过 Send 按钮禁用态和 Response 面板状态传达。

### Mock 服务器

- Mock Manager 页面内提供启动/停止控制。
- 运行状态使用 `secondary` (Mint) 色指示。
- 停止状态使用 `outline` 色。
- 服务状态在 Mock Manager 页面内显示，不放入全局底部状态栏。
- Mock 请求日志在 Mock Manager 的 Traffic 面板展示。
...
```
用户启动 → 侧边栏导航 + 主内容区
           ├── Request Sidebar 导入 OpenAPI → Routes 列表刷新
           ├── Mock Manager 启动/停止 Mock 服务器 → 页面内状态更新
           └── 顶栏后续单独重构，不提前放置未接线操作
...
选择端点 → 配置 Status Code/Delay/Response Body
   ...
   → Mock Manager 页面内启动 → secondary 色状态指示
...
Rust AppState (单一数据源)
  ...
  ├── mock_running: bool         → Mock Manager 页面状态
