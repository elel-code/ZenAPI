...
| Auth | Bearer, Basic, API Key, OAuth2, JWT, Digest variants | Bearer, Basic, API Key, JWT, and OAuth2 manual access token implemented; OAuth2 token acquisition/refresh remains future work |
| Headers/Params | Table editor, presets, bulk import | Key/value editor with row add/remove plus bulk header copy/paste |
| Body | none, form-data, urlencoded, raw, binary, GraphQL | none, form-data, urlencoded, raw, binary, and GraphQL implemented |
...
| Runner | Collection runner with environment selection | Sequential runner core, GPUI Run entry, and `zenapi run` CLI implemented; parallel scheduling remains future work |
...
- Existing auth types cover the common MVP path, including manually supplied
  OAuth2 access tokens sent as bearer authorization.
- OAuth2 token acquisition flows, redirect handling, refresh state, secret
  storage, and dedicated UI affordances remain a later dedicated stage.
