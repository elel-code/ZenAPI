...
- [~] Rebuild Collections UI for native JSON/Postman import/export, saving
  ...
  root-level and nested folders, renames/deletes/reorders selected folders,
  includes pre-request/tests text, saves current requests into the selected
  folder or collection root, duplicates/deletes/reorders saved requests, with
  request rename and folder reparenting wired; drag/drop remains pending.
...
- [~] Split reusable controls from `ui/app.slint` into dedicated Slint modules;
  ...
  Response viewer, Sidebar, App navigation, App auxiliary pages with lazy
  auxiliary page mounting and visited-page caching,
  ...
  Team page, Team page workbench, API Keys page,
  ...
  Runner sidebar panel, Runner results panel, Runner results panel workbench,
  Runner report panel,
  Mock Manager page, Mock Manager auxiliary host,
  Mock Manager row components, Mock Manager endpoint panel,
  Mock Manager config panel, Mock Manager config panel workbench,
  Mock Manager rules panel, Mock Manager rules panel workbench,
  Mock Manager traffic panel,
...
| Area | Status | Notes |
|------|--------|-------|
| App shell | Multi-page Slint shell restored | Responsive page stack with the Request page kept mounted, auxiliary pages lazy-mounted on first visit and cached afterward, request tabs preloaded and switched by visibility, compact single-row request address bar, lightweight read-only code blocks, global navigation, compact navigation, request-sidebar OpenAPI import, and overflow scrollbars |
...
| Collections | Domain implemented | Slint baseline supports load/save/export, folder row display/selection, root and nested folder creation, selected-folder rename/delete/reorder, and request restore/rename/duplicate/delete/move/reorder |
