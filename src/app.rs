...
use zenapi::{
    ...
    grpc::{
        GrpcRequestDraft, GrpcServerStreamEvent, GrpcServerStreamingResponse, GrpcUnaryResponse,
        build_grpc_request_draft, format_grpc_method_catalog, invoke_grpc_unary,
        ...
        load_grpc_reflection_descriptors, stream_grpc_server_streaming,
    },
...
use crate::{
    auth::{
        build_auth_entries, fetch_oauth2_token_config, format_basic_auth_config,
        split_basic_auth_config,
    },
    ui::{
...
fn wire_api_key_page_actions(app: &AppWindow) {
    let weak_app = app.as_weak();
...
fn wire_grpc_draft(app: &AppWindow, runtime: Arc<Runtime>) {
    let grpc_stream = Arc::new(Mutex::new(None::<JoinHandle<()>>));
...
fn set_grpc_server_streaming_progress(
    app: &AppWindow,
...
fn summarize_graphql_schema_response(text: &str) -> Result<String> {
    let trimmed = text.trim();
    if trimmed.is_empty() {
        bail!("response is empty");
    }
...
fn normalize_pm_json_type_name(value_type: &str) -> Option<String> {
...
fn split_js_arguments(input: &str) -> Vec<&str> {
...
fn join_json_path(base: &str, property: &str) -> String {
    if base.is_empty() {
        property.to_string()
    } else {
